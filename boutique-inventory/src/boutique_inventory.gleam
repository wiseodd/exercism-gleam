...
pub fn item_names(items: Iterator(Item)) -> Iterator(String) {
  iterator.map(items, fn(item: Item) { item.name })
}

pub fn cheap(items: Iterator(Item)) -> Iterator(Item) {
  iterator.filter(items, fn(item: Item) { item.price < 30 })
}

pub fn out_of_stock(items: Iterator(Item)) -> Iterator(Item) {
  iterator.filter(items, fn(item: Item) { item.quantity == 0 })
}

pub fn total_stock(items: Iterator(Item)) -> Int {
  iterator.fold(items, 0, fn(sum: Int, item: Item) { sum + item.quantity })
}
